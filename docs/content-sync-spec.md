...
**Status 2026-08-11: the render half is done.** `/diversity-and-inclusion` is
served from `content/diversity-and-inclusion.json`, generated by `npm run sync`
from the `Diversity and Inclusion` tab. The page reproduces the previous one
exactly, minus the "Under Construction" line.

**Still open, and it is the half that matters:** the pull is manual. Nobody has
wired the scheduled Action (§6), so editing the doc today changes nothing until
a developer runs `npm run sync` and commits. Until that ships, the success
criterion above is unmet — this is a proof that the parse works, not a working
loop. ⬅️ **NEXT**
