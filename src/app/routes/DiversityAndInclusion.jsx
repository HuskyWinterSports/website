import '../../assets/Global.css'

export default function DiversityAndInclusion() {
    return (
        <>
            <section className='big-white-box'>
                <h1>DIVERSITY AND INCLUSION</h1>
                <p>Under Construction! ദ്ദി（• ˕ •マ.ᐟ</p>
            </section>
            <section className='white-stripe'>
                <h2> Support our Stoked Volunteer Instructors!</h2>
                <p>Help us keep the slopes alive! Our dedicated instructors volunteer their time to teach skiing and snowboarding to enthusiasts of all ages. With your donation, we can provide them with the resources they need to continue sharing their passion for the sport.</p>
                <p>Your donation will go to support our DEI programs, such as Zero to Triple 60 and Spread the Shred, by providing equipment and passes.</p>
                <p>Help us make the mountain accessible to everyone! Please Zelle huskyws@gmail.com or send a check to our address to donate.</p>
            </section>
        </>
    );
}